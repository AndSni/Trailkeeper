...
import com.asnidev.trailkeeper.data.local.toDto
...
object ProjectsRepository {
    ...
    /** Projects already cached in Room (those the user has opened before).
     * Used as an offline fallback for the project list. */
    suspend fun cachedList(): List<ProjectDto> {
        val orgId = Session.currentOrgId() ?: return emptyList()
        return TrailkeeperDb.db.projectDao().listForOrg(orgId).map { it.toDto() }
    }
